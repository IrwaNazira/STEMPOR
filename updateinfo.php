...
if (isset($_GET['operation'])) {
    try {
        $sqlupdate = "UPDATE steminfo SET programname = '$programname',date= '$date', programdescription = '$programdescription', schoolinvolve = '$schoolinvolve', studentinvolve = '$studentinvolve', company = '$company', companyname = '$companyname'  WHERE schoolname = '$schoolname' AND id = '$id'";
        $conn->exec($sqlupdate);
...
<div class="container">
   <h3 align="center">Update <?php echo $schoolname?> </h3>
    ...
    <form action="updateinfo.php" method="get" align="center" onsubmit="return confirm('Update?');">
        ...
        <br>
        <label for="studentinvolve">Student Involve (In Number)</label><br>
        ...
        <select id="company" name="company" value="<?php echo $company;?>" required><br><br>
        <br>
        <option value="industry"<?php if($company=="industry") echo 'selected="selected"'; ?>>Industry</option>
        <option value="agency"<?php if($company=="agency") echo 'selected="selected"'; ?>>Agency</option>
        <option value="none"<?php if($company=="none") echo 'selected="selected"'; ?>>None</option>
        </select>
        <br>
        <br>   
